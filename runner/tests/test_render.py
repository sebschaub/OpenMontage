...
def test_renders_from_artifacts_when_no_final(tmp_path):
    ...
    class FakeVC:
        def execute(self, inputs):
            ...
            return TR()
    res = render.ensure_final(ws, "instagram_reels", video_compose_factory=FakeVC)
    assert res.ok and res.asset_manifest["assets"][0]["cost_usd"] == 0.1

def test_fails_when_nothing(tmp_path):
    res = render.ensure_final(str(tmp_path), "instagram_reels", video_compose_factory=lambda: 1/0)
